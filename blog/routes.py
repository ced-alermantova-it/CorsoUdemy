...
def homepage():
    ...
    posts = Post.query.order_by(Post.created_at.desc()).paginate(page=page_number, per_page=2, error_out=True)
    if posts.has_next:
        next_page = url_for('homepage', page=posts.next_num)
    else:
        next_page = None
    if posts.has_prev:
        previus_page = url_for('homepage', page=posts.prev_num)
    else:
        previus_page = None
    return render_template("homepage.html", posts=posts, current_page=page_number, 
                           next_page=next_page, previus_page=previus_page)

@app.route("/posts/<string:post_slug>")
def post_detail(post_slug):
    #post_instance = Post.query.get_or_404(post_slu)
...
def post_create():
    ...
    if form.validate_on_submit():
        ...
        new_post = Post(title=form.title.data, body=form.body.data, description=form.description.data, author=current_user, slug=slug)
        if form.image.data:
            try:
                ...
                db.session.commit()
                flash("Problema upload immagine.")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not user.check_password(form.password.data):
            flash('username e password non corrette')
